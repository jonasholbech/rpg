export default class CharacterBuilder extends HTMLElement {
  ...
  _updateForm(){
      const sum = this._player.pointsToDistribute - (Number(this.nodes.inputStr.value) + Number(this.nodes.inputDex.value) + Number(this.nodes.inputCon.value))
      this.nodes.pointsLeft.textContent=sum;
      this.nodes.inputSubmit.disabled=true;
      if(this.nodes.inputStr.value>0 
